...
if platform.system() == "Darwin":
    try:
        ...
        if major < 13:
            ...
            from dispatch import dispatch_queue_create, DISPATCH_QUEUE_SERIAL  # type: ignore[import-untyped]
...
# ---------------------------------------------------------------------------
# High-level SystemAudioRecorder
# ---------------------------------------------------------------------------
...
class SystemAudioRecorder:
    ...
    def __init__(self, sample_rate: int = 16_000, channels: int = 1):
        if not _AVAILABLE:
            raise RuntimeError(f"ScreenCaptureKit is not available: {_IMPORT_ERROR}")
    ...
    def start(self) -> None:
        ...
        self._queue = dispatch_queue_create(
            b"on_the_record.audio", DISPATCH_QUEUE_SERIAL
        )
